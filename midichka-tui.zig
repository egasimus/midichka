...
const inputLabels  = [_][1]u8{ .{'1'}, .{'2'}, .{'3'}, .{'4'}, .{'5'}, .{'6'}, .{'7'}, .{'8'}, };
const outputLabels = [_][1]u8{ .{'A'}, .{'B'}, .{'C'}, .{'D'}, .{'E'}, .{'F'}, .{'G'}, .{'H'}, };
...
pub const TUI = struct {
    ...
    fn render_terminal_too_small(self: *TUI) !void {
        try self.rc.setAttribute(.{ .fg = .red, .bold = true });
        try self.rc.writeAllWrapping("Terminal too small!");
    }

    fn write(self: *TUI, row: usize, col: usize, width: usize, text: []const u8, sel: bool) !void {
    ...
    fn render_inputs(self: *TUI, row: usize, col: usize) !void {
    ...
    fn render_outputs(self: *TUI, row: usize, col: usize) !void {
    ...
    fn render_matrix(self: *TUI, row: usize, col: usize) !void {
    ...
    fn render_mapper(self: *TUI, row: usize, col: usize) !void {
    ...
    pub fn handle(self: *TUI) !void {
        ...
        while (it.next()) |in| {
            ...
            if (self.on_section() or self.on_port() or try self.on_toggle()) {
                try self.render();
                continue;
            }
        }
